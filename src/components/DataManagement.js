...
export async function DropCourse(e) {
    ...
    const resp = await fetch(`api/admin/get-all-courses?course_code=${node.children[1].textContent}`);
    const data = await resp.json();
...
export async function ViewStudentsOnCourse(e) {
    let node = e.target
    while (node.className == "") node = node.parentNode;
    const resp1 = await fetch(`api/admin/get-all-courses?course_code=${node.children[1].textContent}`);
